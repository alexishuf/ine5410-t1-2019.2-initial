...
# Ajusta #include em semaphore.h
SEM_H_PATH:=$(shell $(CC) -E macos-semaphore-shim.c | grep '.*semaphore.h' | head -n 1 | sed -e 's/^[^"]*"//' | sed -e 's/".*//')
$(shell sed -i -e 's@#include.*@#include <$(SEM_H_PATH)>@' semaphore.h)
